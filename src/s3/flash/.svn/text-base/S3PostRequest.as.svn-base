...
/**
 * Modded by Chris Purvis 3/17/2010
 * 
 * When uploading ByteArrays, the Progress & DataUploadComplete events do not work, and the
 * HttpStatus event will get fired before the URLLoader has finished uploading its data.
 * So the new onByteArrayHttpStatus can only throw an error event, not an UploadComplete event.
 * The UploadComplete event is now only thrown by the onComplete handler.
 */
...
package s3.flash {
	...
	public class S3PostRequest extends EventDispatcher {
		...
		public function uploadByteArray(data:ByteArray):void {
			...
			/** CP - uploadComplete and Progress events don't work for the byteArray method. Since this 
			 * isn't an upload via filereference, the Progress event measures the Progress of the S3
			...
			urlLoader.addEventListener(HTTPStatusEvent.HTTP_STATUS, onByteArrayHttpStatus);
		...
		/**
		 * CP - only return an IOError, since the status event doesn't indicate that the upload is complete for the
		 * ByteArray method.
		 */
		private function onByteArrayHttpStatus(event:HTTPStatusEvent):void {
            _httpStatusErrorReceived = true;
			if(Math.floor(event.status/100) == 2) {
                //this.dispatchEvent(new DataEvent(DataEvent.UPLOAD_COMPLETE_DATA, event.bubbles, event.cancelable, "Amazon S3 returned HTTP status " + event.status.toString() + "."));
			} else {
