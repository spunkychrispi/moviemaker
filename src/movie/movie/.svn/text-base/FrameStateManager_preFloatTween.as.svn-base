package movie.movie
{
	...
	public class FrameStateManager
	{
				...
				for (var propertyName:String in properties) {
					...
					if (tweenPropertiesList && tweenPropertiesList.indexOf(propertyName) > -1)
					{
						if (mostRecentKeyFrame) 
						{
							...
							this._tweenProperty(assetName, propertyName, mostRecentProperties[propertyName] as int, mostRecentKeyFrame, properties[propertyName] as int, frame);
						}
						...
						if (nextKeyFrame) 
						{
							...
							{
								this._tweenProperty(assetName, propertyName, properties[propertyName] as int, frame, nextProperties[propertyName] as int, nextKeyFrame);
							}
						}
					}
		...
		protected function _tweenProperty(assetName:String, propertyName:String, valueA:int, frameA:int, valueB:int, frameB:int)
		{	
			var tweenValue:int;
			var tweenFrame:int;
