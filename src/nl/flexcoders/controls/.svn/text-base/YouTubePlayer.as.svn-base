 ...
 * LOL! I'm so Funny.
 * 
 * Mods by Chris Purvis labeled with CP 4/6/2010
 * 
 * */
package nl.flexcoders.controls{
	...
	public class YouTubePlayer extends UIComponent{
		...
		override protected function createChildren():void{
			...
			// CP - if we already had the loader in memory, then we don't need to load it again
			if (! loader.content) {
			//if (true) {
		...
		private function onPlayerReady(event:Event=null):void {
		    ...
		    //trace(player.getVideoEmbedCode());
		    //var code:String = player.getVideoEmbedCode();
		    
		    this.invalidateProperties();
		    this.invalidateDisplayList();
		    
		    // CP 3/6/2010 to be able to catch the onReady firing, but after this
		    // function has been called so that the player has been set.
		    this.dispatchEvent(new MovieEvent(MovieEvent.YOUTUBE_PLAYER_READY));
		    //this.dispatchEvent(new MovieEvent(MovieEvent.YOUTUBE_PLAYER_READY));
		}
